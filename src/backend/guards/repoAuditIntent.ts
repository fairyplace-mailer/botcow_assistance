export function looksLikeRepoAuditRequest(text: string): boolean {
  ...
  const hasAuditPhrase =
    /\b(full audit|audit this repo|audit repo|audit repository|audit code|audit the code|audit codebase|repo audit|repository audit|spec audit|read-?only audit|compliance check|check compliance|verify compliance)\b/i.test(
      text,
    ) ||
  ...
  const hasAuditAgainstSpecPhrase =
    /\baudit\b.{0,60}\b(docs\/strong_spec\.md|strong_spec|responses api|strict mode)\b/i.test(text) ||
    /\b(docs\/strong_spec\.md|strong_spec|responses api|strict mode)\b.{0,60}\baudit\b/i.test(text) ||
    /аудит.{0,60}(docs\/strong_spec\.md|strong_spec|responses api|strict mode)/i.test(text) ||
    /(docs\/strong_spec\.md|strong_spec|responses api|strict mode).{0,60}аудит/i.test(text);
  ...
  return (
    (hasAuditPhrase && (hasRepoScope || hasReadOnlyAuditConstraint)) ||
    hasSpecCheckPhrase ||
    hasAuditAgainstSpecPhrase
  );
}
